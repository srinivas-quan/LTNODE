...
class DUN(BaseNet):
    def __init__(self, model, prob_model, N_train, lr=1e-2, momentum=0.5, weight_decay=0,
        ...
        if self.regression:
            ...
            #print("regression",self.model.output_dim)
        else:
    ...
    def create_net(self):
        if self.cuda:
            #self.model.fcnn = nn.DataParallel(self.model.fcnn)
            #self.model.Node = nn.DataParallel(self.model.Node)
            #self.model.fc_layers = nn.DataParallel(self.model.fc_layers)
            self.model.cuda()
    ...
    def create_opt(self):
        #print(self.model)
        if self.regression:
            param_list = list(self.model.fcnn.parameters())  + list(self.model.Node.parameters()) + list(self.model.fc_layers.parameters()) #for toy data
        else:
            param_list = list(self.model.downsampling_layers.parameters())   + list(self.model.Node.parameters()) + list(self.model.fc_layers.parameters()) #for real world
        #param_list = list(self.model.Node.parameters()) + list(self.model.fc_layers.parameters()) #for concentric data
        print("training wrappers.py",list(self.prob_model.parameters()))
        ...
        #self.optimizer = torch.optim.Adam([{'params':param_list},{'params':self.model.Encoder.parameters(),'lr':0.0001}], lr=self.lr)
        if self.regression:
            self.optimizer = torch.optim.SGD([{'params':param_list},{'params':self.model.Encoder.parameters(),'lr':0.0001,'weight_decay':1e-4}], lr=self.lr, momentum=self.momentum, weight_decay=self.weight_decay)
            #for images encoder lr = 0.01,wd = 5e-4, for in-bet 0.0001 and 1e-4
            self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, step_size=1000, gamma=0.1)
        else:
            self.optimizer = torch.optim.SGD([{'params':param_list},{'params':self.model.Encoder.parameters(),'lr':0.01,'weight_decay':5e-4}], lr=self.lr, momentum=self.momentum, weight_decay=self.weight_decay)
    ...
    def fit(self, x, y):
        ...
        if self.regression:
            means, model_stds = depth_gamma.marginalise_d_predict(
            ...
            probs = depth_gamma.marginalise_d_predict(act_vec.data, self.prob_model.current_posterior,
    ...
    def eval(self, x, y):
        ...
        with torch.no_grad():
            ...
            act_vec = self.model.forward_test(x)
            #y = y[indeces]

            if self.regression:
                means, model_stds = depth_gamma.marginalise_d_predict(act_vec.data,
    ...
    def predict(self, x, depth=None, get_std=False, return_model_std=False,samples=None):
        self.model.eval()
        with torch.no_grad():
            x, = to_variable(var=(x,), cuda=self.cuda)
            # if depth is None:
            #     depth = self.model.n_layers
            act_vec = self.model.forward_test(x,samples=samples).data
           
            if get_std:
                print("in predict trainng wrappers, act_Vec.shape",act_vec.shape)
                pred_mu, model_std = depth_gamma.marginalise_d_predict(act_vec, self.prob_model.current_posterior_pdf,
                                                                             depth=depth,
                ...
                probs = depth_gamma.marginalise_d_predict(act_vec, self.prob_model.current_posterior, depth=depth,
                                                                softmax=(not self.regression), get_std=get_std)
                return probs.data

    def predict_with_grad(self, x, depth=None, get_std=False, return_model_std=False,samples=None):
        #self.model.eval()
        #x, = to_variable(var=(x,), cuda=self.cuda)
        
        act_vec = self.model.forward_test(x,samples=samples,regenerate_graph=True)
        probs = depth_gamma.marginalise_d_predict(act_vec, self.prob_model.current_posterior, depth=depth,
                                                                softmax=(not self.regression), get_std=get_std)
        return probs.data
    def fast_predict(self, x):
        ...
        with torch.no_grad():
            ...
            probs = depth_gamma.marginalise_d_predict(act_vec, self.prob_model.current_posterior, depth=None,
...
class DUN_VI(DUN):
    ...
    def fit(self, x, y):
        ...
        #y = y[indeces]
        #prior_loglikes = self.model.get_w_prior_loglike(k=None)
        #for DUN it is vector of all zeros, need a thought
        prior_loglikes = 0

        ELBO = self.prob_model.estimate_ELBO(prior_loglikes, act_vec, y, self.f_neg_loglike, self.N_train, Beta=1)

        #print("ELBO loss in trainign wrappers",ELBO)
        #print("training wrapppers",self.N_train)
        loss = -ELBO / (self.N_train)#*act_vec.shape[0])
        loss.backward()
        self.optimizer.step()
        with torch.no_grad():
            self.prob_model.q_a[:] = self.prob_model.q_a.clamp(0.0, 100)
            self.prob_model.q_b[:] = self.prob_model.q_b.clamp(0.0, 100)
        #self.prob_model.current_posterior = self.prob_model.get_q_probs()
        self.prob_model.current_posterior = self.prob_model.samples
        self.prob_model.current_posterior_pdf = self.prob_model.get_q_probs()#get_pdf(self.prob_model.samples)#
        '''
        for param in self.model.parameters():
            print(param.data)
        '''
        if self.regression:
            lik,kl = self.prob_model.estimate_lik_kl(prior_loglikes, act_vec, y, self.f_neg_loglike, self.N_train, Beta=1)
            means, model_stds = depth_gamma.marginalise_d_predict(act_vec.data, self.prob_model.current_posterior_pdf,
                                                                        depth=None, softmax=(not self.regression),
                                                                        get_std=True)
            mean_pred_negloglike = self.f_neg_loglike(means, y, model_std=model_stds).mean(dim=0).data
            #print("training wrappers means and y",means[0:10],y[0:10])
            err = rms(means, y).item()
        else:
            lik,kl = self.prob_model.estimate_lik_kl(prior_loglikes, act_vec, y, self.f_neg_loglike, self.N_train, Beta=1)
            probs = depth_gamma.marginalise_d_predict(act_vec.data, self.prob_model.current_posterior_pdf,
                                                            depth=None, softmax=(not self.regression))
            mean_pred_negloglike = self.f_neg_loglike_test(torch.log(probs), y).mean(dim=0).data
            pred = probs.max(dim=1, keepdim=False)[1]  # get the index of the max probability
            err = pred.ne(y.data).sum().item() / y.shape[0]
        ...
        return ELBO.data.item(), mean_pred_negloglike.item(), err , lik, kl
