...
#from torch_ACA_old import odesolve_endtime as odesolve
...
class Net(nn.Module):
    ...
    def forward(self, x,samples=None):
        ...
        max_T = samples.max()
        temp = x.new_zeros(out.shape)#used to store feature vector of each data point at its own time sample.
        ...
        #for param in self.fc_layers.parameters():
            #print(param)
        #print(temp.shape,act_vec.shape)
        '''
        for i in range(x.shape[0]):
        ...
        '''
        del temp
        return act_vec#,indeces
    def forward_test(self, x,samples=None,regenerate_graph=False):
        out = self.downsampling_layers(x)
        alpha_beta = self.Encoder(x)
        ...
        #print("test samples.shape",samples.shape,self.n_samples)
        #samples = samples[0,:]
        #samples[samples>=50]= 50

        max_T = samples.max()
        Shape = [self.n_samples]
        for s in range(len(out.shape)):
            Shape.append(out.shape[s])
        temp = x.new_zeros(Shape)
        
        #temp[0,:,:] = self.fc_layers(x)#if for any sample 'x',T is less than dt, x will be returned, 
        
        self.prob_model.current_posterior = samples
        self.prob_model.samples = samples
        #self.prob_model.current_posterior_pdf = self.prob_model.get_q_probs()
        t = 0
        i = 1
        #print("max_T,max_T/self.deltat,int(max_T/self.deltat)",max_T,max_T/self.deltat,int(max_T/self.deltat))
        ...
        for i in range(self.n_samples):
            ...
            options.update({'t0': t})
            #options.update({'t1': sample.detach().cpu().numpy()[0]})
            if regenerate_graph == True:
                options.update({'t_eval':list(samples1.detach().cpu().numpy())})
            else:
            ...
            options.update({'print_neval': False})
            options.update({'neval_max': 5000})
            options.update({'regenerate_graph': regenerate_graph})
            out1 = odesolve(self.Node, out, options) #out is mxmxc, 
            #print(regenerate_graph)
            for j in range(x.shape[0]):
                #temp[indeces[i],:] = out[i,i,:]
                #temp[indeces[j],:] = out1[j,indeces[j],:]
                #act_vec[i,indeces[j],:] = out1[j,indeces[j],:]
                temp[i,indeces[j],:] = out1[j,indeces[j],:]
                #act_vec[i,indeces[j],:] = self.fc_layers(out1[j,indeces[j],:])
                #temp[i,:] = out[i,i,:]
            #print(temp[i].shape)
            act_vec[i,:] = self.fc_layers(temp[i,:].clone())
        #for param in self.fc_layers.parameters():
