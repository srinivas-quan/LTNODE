...
class Flatten(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        datadir = './results2'
        count_array = np.load(datadir+'/CIFAR10_count.npy')

        count_array[0] += x.shape[0]
        #print(count_array[0])
        
        #if count_array[0]<=2560 or (count_array[0] >(260320)  and count_array[0]<=(260320+2560 )):
        #print("storing")

        dummy_array = np.load(datadir+'/CIFAR10_features.npy')
        dummy_array = np.concatenate((dummy_array,x.view(-1, shape).detach().cpu().numpy()))
        np.save(datadir+'/CIFAR10_features.npy',dummy_array)
    
        np.save(datadir+'/CIFAR10_count.npy',count_array)
        '''
        #########Used for extracting features###################
...
class Net(nn.Module):
    ...
    def forward(self, x,samples=None):
        ...
        #print(self.prob_model.current_posterior_pdf,samples)
        t = 0
        i = 0
        '''
        ...
        if samples.requires_grad == True:
            options.update({'t_eval':list(samples.detach().cpu().numpy())})
        else:
        ...
        out = odesolve(self.Node, out, options) #out is n_samplesxbatch_sizexc, 
        #temp = x.new_zeros(out.shape)
        #for i in range(self.n_samples):
        #    temp[indeces[i],:] = out[i,:]
            #print("indeces[i].......",indeces[i])
        Shape = [out.shape[0]*out.shape[1]]
        for s in range(2,len(out.shape)):
            Shape.append(out.shape[s])
        #print(out.shape,Shape)
        out = out.reshape(Shape) #
        act_vec = self.fc_layers(out)# n_samples*batch_sizexc
        #del temp
        return act_vec.reshape([self.n_samples, x.shape[0], self.num_classes])
...
def simple1(num_classes,n_samples,input_chanels): # To test a NODE model with different T, not any 
